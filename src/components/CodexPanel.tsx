...
export function CodexPanel(props: CodexPanelProps) {
  ...
  const sandboxCheckLabels = { appServer: "Codex App Server", windowsSandbox: "Windows Sandbox", commandExecution: "Sandbox内コマンド実行", workspaceWrite: "ワークスペース編集" } as const;
  ...
  const environmentMessage = sandbox?.state === "unavailable"
    ? "Codexとの会話は利用できますが、Sandbox内のコマンド実行またはワークスペース編集を確認できませんでした。"
    : sandbox?.state === "limited"
      ? "Codexとの会話と編集は利用できる可能性がありますが、Windows Sandboxの準備状態に確認事項があります。"
