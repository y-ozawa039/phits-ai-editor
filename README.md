...
- Windows 11 x64（`0.0.5-alpha`の配布対象）
...
- PHITS用Codex設定とCodex編集環境の段階別診断（コマンド実行、ワークスペース直下の新規作成、既存相当ファイルの変更、子フォルダーへの作成、アクセス規則）、検査理由・実方式の表示、明示操作によるSandbox再セットアップ、および診断結果に基づく「生成AIへの相談文」の作成・コピー
...
現在のalpha版の概要は[リリースノート](docs/release-notes-v0.0.5-alpha.md)、
制限事項は[既知の問題](docs/known-issues-v0.0.5-alpha.md)を参照してください。
