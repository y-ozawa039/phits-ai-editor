...
- Codex編集環境のアクセス規則診断は読取り専用です。Sandbox再セットアップは、設定失敗、
  または`unelevated`方式で実書込みを確認できない場合にだけ、利用者の明示操作と確認を経て
  App Serverの公式要求を使用します。許可されている場合は`elevated`を推奨し、
  `unelevated`でのセットアップも利用者が選べます。
