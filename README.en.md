...
- four approval levels: Confirm first, Consultation only, Confirm when needed,
  and Autonomous (workspace only)
- Codex-requested normal PHITS runs through the editor-owned Rust Runner and
  official wrapper after validation of the selected saved input, PHITS
  environment, and duplicate-run state, with the result returned to the same turn
