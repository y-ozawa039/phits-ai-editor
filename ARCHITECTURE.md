...
Sandbox設定の失敗、または`unelevated`方式の実書込み失敗では、利用者の明示操作と確認後に
App Serverの`windowsSandbox/setupStart`を呼び出せます。許可されていれば`elevated`を推奨し、
`unelevated`の再試行も選べます。アクセス規則や所有権をエディタが直接変更することはありません。
