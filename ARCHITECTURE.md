...
Codex編集環境の実動作検査は、実際のturnと同じ`workspaceWrite`方針で、書込みを伴わない
コマンド、ワークスペース直下への新規作成、エディタが事前作成した既存相当ファイルの変更、
エディタが事前作成した子フォルダー内への新規作成を個別に試します。アクセス規則の検査は
継承停止・明示的な拒否・reparse pointの有無を読取り専用で確認し、実書込みが成功している
場合はそれだけで警告にしません。失敗はSandbox設定、ワークスペースのアクセス規則、
コマンド起動、結果検証に分類します。明示した書込みルートと現在作業フォルダーを基準にする
方式を安全な順序で検査し、全項目に成功した方針だけを同じ接続の通常turnへ引き継ぎます。
実書込みを確認できない場合は、Schemaが利用可能でも編集モードをRust側で相談専用に制限します。
この結果と実際のCodex編集経路が一致しない可能性を切り分けるため、利用者の明示操作で、専用の
一時ファイルと非永続スレッドを使う任意診断を実行できます。診断は現在選択中のモデル・思考を
用います。開始前に作成・書換え・削除の内容を説明し、利用者が開始した後、App Serverからの
変更要求が診断用ファイルと一致することをRust側で照合して自動承認します。
対象外ファイルとコマンド実行は拒否します。成功時は同じ接続中だけ
編集モードを有効にします。利用者は自動診断によるエディタ側の制限だけを現在のワークスペースと
接続中に限って解除できますが、Sandbox、承認設定、ワークスペース境界、PHITS実行検証は変更しません。
アクセス規則、所有権、Sandbox設定をエディタが直接変更することはありません。

ワークスペースを開いた後、ドライブ種別、ファイルシステム、UNC、reparse point、
読み取り専用属性、パス長、OneDrive対象を再帰走査なしで確認します。結果は現在の
プロセスでの表示にだけ保持し、ワークスペースや設定へキャッシュしません。Codexの
Schema検査とSandbox実動作検査も、保存済み結果を利用可否判断へ再利用しません。同時に
到着した同一検査だけを実行中の処理へ合流させます。

起動ログはAppData配下で容量を制限して1世代ローテーションし、起動・基本診断・Codex接続の
段階と失敗分類だけを記録します。ワークスペースパス、入力内容、認証情報は記録しません。
診断レポートは利用者が保存操作を行った時だけ生成します。保存画面では既知のローカルパスを
伏せる方法を初期選択とし、整形途中で空白が挿入された既知のパスと、検出できるその他の
ローカル絶対パスも置換します。パスを残す方法も明示的に選択できます。通常の実行環境パネルには
保存操作を置かず、警告またはヘルプメニューから開きます。
...
Codexからの通常実行要求は、現在の接続だけに追加するローカルMCPサーバーを介して
エディタへ戻します。MCP子プロセス自体にはPHITSを起動させず、認証付きloopback接続で
Rust境界へ要求を転送します。Rustは現在のターン、承認モード、保存状態、入力パス、
PHITSルート、公式ラッパー、実行ロックを検査し、許可された場合だけ既存Runnerを実行して
結果を同じCodexターンへ返します。グローバルCodex設定は変更しません。

App ServerのMCP確認要求とツール入力要求は`codex_interaction.rs`で検査・応答します。
同じターンで記録した専用ツール項目・引数と一致するMCP承認は実行審査へ転送し、
それ以外は既存の承認キューで確認・回答を求めます。未対応の要求はmethodと識別情報を
診断ログへ返し、ユーザーが拒否した場合と区別します。回答送信の前にRust側で
質問ID・選択肢・フォーム型と対応する制約を検証し、無効な回答は保留状態を維持します。
