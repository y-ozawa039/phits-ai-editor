...
import type { ApprovalDecision, ApprovalMode, ApprovalRequest, CodexModel, CodexSandboxProbeReport, CodexThreadLink, PhitsAgentSetupStatus } from "../types";
...
export function CodexPanel(props: CodexPanelProps) {
  ...
  const environmentMessage = sandbox?.state === "unavailable"
    ? sandbox.failureCategory === "workspacePermissions"
      ? "Sandbox自体とは別に、このフォルダー固有のアクセス規則が影響している可能性があります。"
  ...
  const setupNeedsExplanation = environmentNeedsAttention;
  ...
  return <aside className="codex-panel" style={{ width: props.width, "--codex-font-size": `${props.fontSize ?? 14}px` } as CSSProperties} aria-label="Codex">
    ...
    <div className="codex-controls">
      {environmentNeedsAttention && <div className={`codex-compatibility-note codex-agent-setup-note ${sandbox?.state ?? agentSetup?.state ?? "partial"}`} role={environmentIsAlert ? "alert" : "status"}><strong>{environmentTitle}</strong><span>{environmentMessage}</span><details><summary>診断項目と検査理由</summary>{agentSetup && <div className="codex-diagnostic-group"><b>PHITS参照設定</b><ul>{agentSetup.checks.map((check) => <li className={`agent-setup-check ${check.state}`} key={check.id}><b>{agentCheckLabels[check.id]}</b><span>{check.message}</span>{check.path && <code>{check.path}</code>}</li>)}</ul></div>}{sandbox && <div className="codex-diagnostic-group"><b>Codex編集環境</b><ul>{sandbox.checks.map((check) => <li className={`agent-setup-check ${check.state}`} key={check.id}><b>{sandboxCheckLabels[check.id]}</b><span>{check.detail}</span></li>)}<li className={`agent-setup-check ${sandbox.implementation ? "confirmed" : "partial"}`}><b>Sandbox方式</b><span>{sandbox.implementation ?? "取得できませんでした"}</span></li><li className="agent-setup-check confirmed"><b>組織ポリシーの許可方式</b><span>{sandbox.allowedImplementations.length ? sandbox.allowedImplementations.join(", ") : "制限指定なし"}</span></li></ul></div>}</details>{sandbox?.setupRecommended && props.onSandboxSetup && <div className="troubleshooting-actions"><button className="secondary-button" onClick={props.onSandboxSetup} disabled={props.sandboxSetupBusy || props.sandboxBusy}>{props.sandboxSetupBusy ? "Sandboxを再セットアップ中…" : "Sandboxを再セットアップ"}</button><span>確認画面が表示される場合があります。完了後に自動で再診断します。</span></div>}{props.sandboxBusy && <span aria-live="polite">Codex編集環境を診断中…</span>}{troubleshootingEditor}</div>}
