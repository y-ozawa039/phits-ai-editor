...
コマンド起動、結果検証に分類します。Sandbox設定の失敗と判定した場合に限り、利用者の
明示操作と確認後にApp Serverの`windowsSandbox/setupStart`を呼び出し、完了通知を受けて
再診断します。アクセス規則、所有権、Sandbox方式をエディタが自動変更することはありません。
...
伏せる方法を初期選択とし、パスを残す方法も明示的に選択できます。通常の実行環境パネルには
